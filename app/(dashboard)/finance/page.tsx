...
export default function FinancePage() {
  ...
  useEffect(() => {
    const mockInvoices: Invoice[] = [
      ...
      },
      {
        ...
        grandTotal: 12000,
        status: "Paid",
        lineItems: [{ testCode: "RFT", quantity: 1, total: 12000 }],
      },
      {
        id: "INV-003",
  ...
  filteredInvoices.forEach((invoice) => {
    invoice.lineItems.forEach((item: LineItem) => {
      ...
      const existing = testTypeMap.get(category) || {
        income: 0,
        cost: 0,
        count: 0,
      };
        ...
        <div className="flex items-center justify-between bg-white rounded-xl shadow-sm p-6 border-l-4 border-teal-500">
          <div>
            ...
            <p className="text-gray-600 ml-15">
              Income tracking and profitability analysis
            </p>
          </div>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <Card className="border-teal-100 shadow-md hover:shadow-lg transition-shadow duration-300">
            ...
            <CardContent className="pt-4">
              ...
              <p className="text-xs text-gray-600 mt-1">
                LKR {totalProfit.toFixed(2)} profit
              </p>
            </CardContent>
          </Card>
        </div>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
                      ...
                      cy="50%"
                      labelLine={false}
                      label={({ testType, percent }) =>
                        `${testType} ${((Number(percent) || 0) * 100).toFixed(
                          0
                        )}%`
                      }
                      outerRadius={100}
